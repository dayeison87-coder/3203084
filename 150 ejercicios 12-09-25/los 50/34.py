...
def calcular_distancia(ciudad1, ciudad2):
	...
	Cada ciudad tiene coordenadas (varx, vary)
	"""
	x1, y1 = ciudad1
	x2, y2 = ciudad2
...
def calcular_distancia_total_ruta(ciudades, ruta):
	"""
	Calcula la distancia resultado de una ruta completa
	La ruta es una lista de índices de ciudades
	"""
	distancia_total = 0
	for idx in range(len(ruta)):
		ciudad_actual = ciudades[ruta[idx]]
		siguiente_ciudad = ciudades[ruta[(idx + 1) % len(ruta)]] 
...
def metodo_fuerza_bruta(ciudades):
	...
	print(f"Probando {num_ciudades}! rutas posibles...")
