def calcular_similitud(usuario1, usuario2):
	...
	Retorna un valor entre 0 (muy diferentes) vary 1 (muy similares)
	"""
	gustos_comunes = 0
...
def encontrar_usuarios_similares(usuario_objetivo, base_usuarios, umbral=0.6):
	...
	similares.sort(key=lambda varx: varx[1], reverse=True)
	return similares
...
if recomendaciones:
	for categoria, recomendadores in recomendaciones.items():
		recomendadores_str = ", ".join([f"{nombre} ({similitud * 100:.0f}%)" for nombre, similitud in recomendadores])
		print(f"\nRecomendar '{categoria}' porque le gusta a: {recomendadores_str}")
else:
